...
Note that from macOS 10.14 "Mojave", Apple have added a new security framework
which means the vncagent will run in view-only mode until accessibility access
is granted on each device to which it is deployed.
https://help.realvnc.com/hc/en-us/articles/360016134372
...
listen for direct connections (UDP or TCP) on a (known, static) IP address 
and (dedicated, open) port. Or enable the sample app to do both.
...
./Debug/basicServer <Cloud-addr> <Cloud-pwd> <ip-port> 
