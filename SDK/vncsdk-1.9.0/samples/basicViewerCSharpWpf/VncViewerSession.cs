...
namespace BasicViewerCSharpWpf
{
    public class VncViewerSession
    {
        ...
        //
        // Or direct connectivity properties
        //
        ...
        private DisplayManager.Callback DisplayManagerCallback { get; set; }
        ...
        public void Run()
        {
            try
            {
                ...
                else
                {
                    ...
                    NewStatus($"Connecting to host address: {DirectConnectivityAddress} port: {IpPort}");
                    using (DirectUdpConnector udpConnector = new DirectUdpConnector())
                    {
                        udpConnector.Connect(DirectConnectivityAddress, IpPort, Viewer.GetConnectionHandler());
                    }
                }
        ...
        private void SetUpCallbacks(Viewer viewer)
        {
            ...
            DisplayManagerCallback = new DisplayManager.Callback(
                displaysChanged: OnDisplaysChanged);
            ...
            viewer.GetDisplayManager().Enable(true);
            viewer.GetDisplayManager().SetCallback(DisplayManagerCallback);
        ...
        public void NextDisplay()
        {
            RunInSession(() =>
            {
                int index = Viewer.GetDisplayManager().GetDisplayIndex();
                index++;
                if (index >= Viewer.GetDisplayManager().GetDisplayCount())
                {
                    index = 0;
                }
                Viewer.GetDisplayManager().SelectDisplay(index);
            });
        }
        ...
        public void LastDisplay()
        {
            RunInSession(() =>
            {
                ...
                if (index < 0)
                {
                    index = Viewer.GetDisplayManager().GetDisplayCount() - 1;
                }
                Viewer.GetDisplayManager().SelectDisplay(index);
            });
        }

        private void OnDisplaysChanged(DisplayManager displayManager)
        {
            NewStatus("Display count: " + displayManager.GetDisplayCount());
        }
        ...
        /// <summary>
        /// Return true if the settings infer to use cloud, false if the settings infer direct connectivity
        ...
        public bool? InferredUsingCloud
        {
            get
            {
                ...
                bool GotDirectConnectivitySettings = !string.IsNullOrEmpty(DirectConnectivityAddress) && IpPort != 0;

                // If one of the settings allows us to infer which connection to use then return that
                if (GotCloudSettings != GotDirectConnectivitySettings)
        ...
        {
            RunInSession(() => Viewer.EnableAudio(enable));
        }
